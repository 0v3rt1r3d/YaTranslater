...
public class Downloader
{
    ...
    private interface TranslateService
    {
        ...
        Call<String> getDictionary(@Query("text") String text, @Query("lang") String direction);
    }
    ...
    public String getLanguagesResponse(String ui)
    {
    ...
    public Dictionary getDictionary(Context context, String text, String direction)
    {
        ...
        try
        {
            response = service.getDictionary(text, direction).execute().body();
            if(response == null)
            {
                return null;
            }
...
//                Перебор переводов
                for (int j = 0; j < jTranslations.size(); j++)
                {
                    ...
                    Variant variant = new Variant(jTranslation
...
//                    Добавление синонимов, если они есть
                    if (jTranslation.has("syn"))
                    {
                        ...
                        {
                            variant.addSynonym(jSynonyms.get(k).getAsJsonObject()
                                    .get("text")
                                    .getAsString());
                        }
                    }
//                    Добавление значений, если есть
                    if (jTranslation.has("mean"))
                    {
                        ...
                        {
                            variant.addMean(jMean.get(k).getAsJsonObject()
                                    .get("text")
                                    .getAsString());
                        }
                    }
//                    Добавление примеров, если они есть
                    if (jTranslation.has("ex"))
                    {
                        ...
                        for (int k = 0; k < jExamples.size(); k++)
                        {
                            ...
                            variant.addExample(new Example(
                    ...
                    dictionary.addTranslation(variant);
                }
            }
        }
        catch (IOException ioe)
